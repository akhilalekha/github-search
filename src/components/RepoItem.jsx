import React from "react";
import { useDispatch } from "react-redux";
import { addFavourites, removeFavourites } from "src/store/favSlice";
import { toggleFav } from "src/store/repoSlice";

export default function RepoItem({ item, type }) {
	const dispatch = useDispatch();

	const handleAddFavourite = (item) => {
		if (type === "toggle") {
			// if item.isFav is false we want to add it, else remove it
			if (!item.isFav) {
				const newItem = {
					...item,
					isFav: true
				};
				dispatch(addFavourites(newItem));
			} else {
				dispatch(removeFavourites(item));
			}
			dispatch(toggleFav(item));
		} else {
			dispatch(removeFavourites(item));
		}
	};

	return (
		<div
			className="mr-2 my-2 p-2 py-2 border border-indigo-400 rounded-md w-36 h-20 shadow-lg text-indigo-400 text-sm font-medium flex flex-col"
			key={item.url}
			data-testid="repo-item"
		>
			<a
				className="text-ellipsis overflow-hidden h-16"
				href={item.url}
				target="_blank"
				rel="noopener noreferrer"
			>
				{item.full_name}
			</a>
			<div className="w-4 h-4 self-end">
				<svg
					xmlns="http://www.w3.org/2000/svg"
					viewBox="0 0 512 512"
					onClick={() => handleAddFavourite(item)}
				>
					<path
						fill={item.isFav ? "rgb(239 68 68)" : "rgb(148 163 184)"}
						d="M0 190.9V185.1C0 115.2 50.52 55.58 119.4 44.1C164.1 36.51 211.4 51.37 244 84.02L256 96L267.1 84.02C300.6 51.37 347 36.51 392.6 44.1C461.5 55.58 512 115.2 512 185.1V190.9C512 232.4 494.8 272.1 464.4 300.4L283.7 469.1C276.2 476.1 266.3 480 256 480C245.7 480 235.8 476.1 228.3 469.1L47.59 300.4C17.23 272.1 .0003 232.4 .0003 190.9L0 190.9z"
					/>
				</svg>
			</div>
		</div>
	);
}
